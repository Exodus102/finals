...
let currentSlide = 0;
    ...
    function showSlide(index) {
        ...
        currentSlide = index;
        const translateValue = -index * 110 + '%';
        wrapper.style.transform = 'translateX(' + translateValue + ')';
    }

    function prevSlide() {
        currentSlide = (currentSlide - 2 + 6) % 6; // 3 is the total number of slides
        showSlide(currentSlide);
    }

    function nextSlide() {
        currentSlide = (currentSlide + 2) % 6; // 3 is the total number of slides
        showSlide(currentSlide);
    }


//Code for onpage navigation at about us part
document.querySelectorAll('.mini-nav a')
  .forEach(e => e.addEventListener('click', _ => change(e.dataset.id)))


function change(n) {
  let panels = document.querySelectorAll('main div')
  panels.forEach(p => p.classList.remove('about-content-clicked'))
  panels[n - 1].classList.add('about-content-clicked')
}
//
